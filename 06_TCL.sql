개ㅣ-- TCL (Transaction Control Language) : 트랜잭션 제어 언어
 ...
 *
 * ...
 * SAVEPOINT "포인트명1"; -- 큰따음표가 없으면 포인트설정이 안됨
 *
 * ...
 * SAVEPOINT "포인트명2";
 *
...
-- COMMIT 후 ROLLBACK 안된다. DB에 이미 반영
SELECT * FROM DEPARTMENT2;
...
SELECT * FROM DEPARTMENT2; -- 개발4팀 남음, 세이브 포인트 2,3도 같이 날라감
...
ROLLBACK TO "SP2";
-- SP2 지점 전에 개발4,5팀이 있었으므로 잔여

ROLLBACK TO "SP1";
-- 개발4팀만 남아있음

ROLLBACK;-- 개발 1,2,3팀만 남음
