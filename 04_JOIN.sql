...
FROM EMPLOYEE
JOIN DEPARTMENT ON(DEPT_CODE = DEPT_ID); -- 연결고리 값이 NULL인 값은 제외하고 출력
...
--> JOIN 구문 잘못 작성하는 경우 CROSS JOIN 의 결과가 조회되기도 함
...
-- 오라클 문법
SELECT EMP_ID 사번, EMP_NAME 이름, JOB_NAME 직급명, DEPT_TITLE 부서명, LOCAL_NAME 지역명, SALARY 급여
FROM EMPLOYEE, JOB, DEPARTMENT, LOCATION
WHERE EMPLOYEE.JOB_CODE = JOB.JOB_CODE AND JOB_NAME = '대리'
AND DEPT_CODE = DEPT_ID 
AND LOCATION_ID = LOCAL_CODE AND SUBSTR(LOCAL_NAME,1,4) = 'ASIA';
...
WHERE DEPT_ID = DEPT_CODE AND EMPLOYEE.JOB_CODE = JOB.JOB_CODE
...
FROM EMPLOYEE, DEPARTMENT, JOB
WHERE DEPT_ID = DEPT_CODE AND EMPLOYEE.JOB_CODE = JOB.JOB_CODE
AND (SUBSTR(DEPT_TITLE,1,5)='해외영업1' OR SUBSTR(DEPT_TITLE,1,5)='해외영업2');
...
SELECT EMP_NAME 사원명, JOB_NAME 직급, SALARY 급여, SALARY*12*(1+BONUS) 연봉
FROM EMPLOYEE, JOB, SAL_GRADE
WHERE EMPLOYEE.JOB_CODE = JOB.JOB_CODE
AND EMPLOYEE.SAL_LEVEL = SAL_GRADE.SAL_LEVEL
AND MIN_SAL < SALARY;
...
AND (NATIONAL_CODE = 'KO' OR NATIONAL_CODE = 'JP');

-- 8. 같은 부서에 근무하는 직원들의 사원명, 부서코드, 동료이름을
--조회하시오.(SELF JOIN 사용)

SELECT EE.EMP_NAME, E.DEPT_CODE, E.EMP_NAME
FROM EMPLOYEE E, EMPLOYEE EE
WHERE EE.DEPT_CODE = E.DEPT_CODE
MINUS
SELECT EE.EMP_NAME, E.DEPT_CODE, E.EMP_NAME
FROM EMPLOYEE E, EMPLOYEE EE
WHERE EE.DEPT_CODE = E.DEPT_CODE
