-- 함수 : 컬럼의 값을 읽어 연산을 한 결과를 반환

-- 단일 행 함수 : N개의 값을 읽어서 연산 후 N개의 결과를 반환

-- 그룹 함수 : N개의 값을 읽어서 연산 후 1개의 결과를 반환 (합, 평균, 최대, 최소)

-- 함수는 SELECT 문의
-- SELECT절, WHERE절, ORDER BY절, GROUP BY절, HAVING 절에서 사용 가능하다
...
-- 문자열을 앞에서부터 검색하여 첫번재 B의 위치를 조회해라
...
-- ROUND(숫자 | 칼럼명 [, 소수점 휘치]) : 반올림
SELECT ROUND(123.456) FROM DUAL; -- 첫번째 자리에서 반올림

SELECT ROUND(123.456,  1) FROM DUAL; -- 두번째 자리에서 반올림하여 첫번째 자리까지 표기
...
SELECT TRUNC(123.456) FROM DUAL; -- 123, 소수점 아래를 ㅈ러삭
...
SELECT SYSDATE FROM DUAL; --2025-03-10 12:12:24.000

-- SYSTIMESTAMP : SYSDATE + MS 단위 추가
SELECT SYSTIMESTAMP FROM DUAL; --2025-03-10 12:12:24.609 +0900
...
SELECT LAST_DAY('2020-02-01') FROM DUAL;ALTER 

-- EXTRACT : 년, 월, 일, 정보를 추출하여 반환
-- EXTRACT(YEAR FROM 날짜) : 년도만 추출
-- EXTRACT(MONT FROM 날짜) : 월만 추출
...
SELECT EMP_NAME, EXTRACT(YEAR FROM HIRE_DATE) ||'년'|| EXTRACT(MONTH FROM HIRE_DATE) ||'월'|| EXTRACT(DAY FROM HIRE_DATE)||'일' AS 입사일 FROM EMPLOYEE;
---------------------------------------------------------
...
-- 숫자 -> 문자 변환 시 포맷 패턴
-- 9 : 숫자 한칸을 의미, 여러개 작성시 오른쪽 정
-- 0 : 숫자 한칸을 의미, 여러개 작성 시 오른쪽 정렬 + 빈칸 0 추가
-- L : 현자 DB에 설정된 나라의 화폐 기호
...
SELECT TO_DATE('250310 140830') FROM DUAL;
-- 리터럴이 형식 문자열과 일치하지 않음
--> 패턴을 적용해서 작성된 문자열의 각 문자가 어떤 날짜 형식인지 인식 시킨다

SELECT TO_DATE('250310 140830', 'YYMMDD HH24MISS') FROM DUAL;
...
SELECT TO_NUMBER('1,000,000', '9,999,999') + 500000 AS 합계 FROM DUAL;

-- NULL 처리 함수
-- NVL(컬렴명, 컬럼값이 NULL 일 때 바꿀 값) : NULL인 컬럼값을 다른 값으로 변경할 때 사용

SELECT EMP_NAME, SALARY, NVL(BONUS, 0), SALARY*BONUS FROM EMPLOYEE;
-- NULL과 산술연산을 하면 무조건 NULL값이 나온다
...
-- DECODE(계산식 | 컬럼명, 조건값1, 선택값, 조건값2, 선택값2...., 아무것도 일치하지 않을 때)
...
SELECT EMP_NAME, DECODE(SUBSTR(EMP_NO, 8, 1),'1', '남성', '2', '여생') AS 성별
FROM EMPLOYEE;
...
-- 그룹 함수
-- 하나 이상의 행을 그룹으로 묵어 연산하여 총합, 평균 등의
-- 하나의 결과 행으로 반환하는 함수
...
-- COUNT(컬럼명) : NULL을 제외한 실제값이 기록된 행의 가수를 리턴
